...
q3 <- read.csv(paste(file_address,"Qubit0.csv",sep = ""),na.strings="NA",header=TRUE)
...
```{r}
dataList.q1 <- list(N = nrow(q1),gp0 = means$V1[3],gp1 = means$V2[3],x = q1$x,y = q1$y)
dataList.q2 <- list(N = nrow(q2),gp0 = means$V1[2],gp1 = means$V2[2],x = q2$x,y = q2$y)
dataList.q3 <- list(N = nrow(q3),gp0 = means$V1[1],gp1 = means$V2[1],x = q3$x,y = q3$y)
...
write.csv(noise_params.q3,paste(file_address,"StandPostQubit0.csv",sep = ""), row.names = FALSE)
```
