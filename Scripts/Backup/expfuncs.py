...
def create_filters(interested_qubits, QDT_correlated, shots_per_point = 1024, seed = 227, show_denoised = False, from_file = False, file_address = ''):
    ...
    if from_file:
        ...
        mf.inference(seed = seed, show_denoised = show_denoised, shots_per_point = shots_per_point)
    ...
    print('QDT filter')
    if QDT_correlated:
        trans_matrix = np.genfromtxt(file_address + 'trans_matrix.csv',delimiter=',')
    else:
        trans_dict = {}
        for q in interested_qubits:
            trans_dict['Qubit{}'.format(q)] = np.genfromtxt(file_address + 'trans_matrix{}.csv'.format(q),delimiter=',')
...
class MeasFilterSB:
    ...
    def mean(self):
        return np.array([np.mean(self.post['Qubit'+str(i)],axis = 0) for i in self.qubit_order]).reshape(len(self.qubit_order),2)
...
class MeasFilterMat:
    ...
    def filter(self,counts):
        ...
        proc_status,proc_vec = find_least_norm(len(list(counts.keys())[0]),nl.solve(self.trans_mat,real_vec))
        if proc_status != 'optimal':
            raise Exception('Sorry, filtering has failed')
...
class MeasFilterQDT:
    ...
    def filter(self,counts):
        ...
        if proc_status != 'optimal':
            raise Exception('Sorry, filtering has failed')
        proc_counts = vecToDict_inv(len(list(counts.keys())[0]),shots,proc_vec)
